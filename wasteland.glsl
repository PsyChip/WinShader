...
vec3 fog(in vec3 col, in vec3 ro, in vec3 rd, in float mt)
{
    ...
    for(int i=0; i<7; i++)
    {
        ...
        col = mix(col,vec3(.85, .65, .5),clamp(rz*smoothstep(d,d*1.8,mt),0.,1.) *sh);
        d *= 1.8;
        if (d>mt)break;
    }
    return col;
