...
vec3 blueNoise(vec2 p, float t) {
    float h = hash(p + t);
    float h2 = hash(p.yx + t + 1.7);
    float h3 = hash(p + t + 3.1);
    return vec3(h, h2, h3);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    vec3 bn = blueNoise(fragCoord, iTime * 0.2);
    vec2 uv = (fragCoord + (bn.xy * 2.0 - 1.0)/3.0) / iResolution.xy;
    uv = uv * 2.0 - 1.0;
    uv.x *= iResolution.x / iResolution.y;
    float time = (iTime - iTimeDelta * bn.z) * 0.04;

    vec3 ang = vec3(sin(time*2.0)*0.1,0.1,-time);
    mat3 rot = fromEuler(ang);
    vec3 ori = vec3(0.0,5.0,60.0);
