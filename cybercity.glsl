...
vec3 addSign(vec3 color, vec3 pos, float side, vec2 id) {
    ...
    flash = smoothstep(0.1, 0.5, 0.5 + 0.5 * cos(flash * iTimeSlow));
    ...
    flicker = 1.0 - flicker * step(0.96, hash1(charId, iTimeSlow));

    float ch = -3.5 + 8.0 * noise(id + 6.0 * charPos);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    vec2 center = -speed * iTimeSlow * cameraDir.xy;
    ...
    float time = lightSpeed * iTimeSlow;
