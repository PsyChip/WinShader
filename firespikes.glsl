...
// Procedural value-noise replacement for IQ's texture-based noise.
float hash3( vec3 p ) {
...
// FBM (reduced from 3 octaves to 2 to lighten GPU load)
float fpn(vec3 p) {
    return pn(p*.06125)*.65 + pn(p*.25)*.30;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
   ...
   for (int i=0; i<36; i++) {
      ...
      #else
      // add in noise to reduce banding and create fuzz (intensified)
      d=abs(d)*(.6+0.7*rand(seed*vec2(i)));
      // enforce minimum stepsize
      d = max(d, 0.04);
      #endif
      #else
...
void main() {
    // Horizontal scanline skip: every other row black, saves 50% GPU
