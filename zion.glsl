...
void main() {
    // CRT scanline skip — saves ~55% GPU
    vec2 fc = gl_FragCoord.xy;
    float mx = mod(fc.x, 3.0);
    float my = mod(fc.y, 3.0);
    if (mx < 1.0 || my < 1.0) {
    ...
    // Phosphor subpixel tint
    vec3 c = fragColor.rgb;
    float sub = mod(fc.x, 9.0);
    if      (sub < 3.0) c *= vec3(1.2, 0.8, 0.8);
    else if (sub < 6.0) c *= vec3(0.8, 1.2, 0.8);
    else                c *= vec3(0.8, 0.8, 1.2);
    // Scanline dimming
    c *= 0.7 + 0.3 * smoothstep(0.0, 1.0, my / 3.0);
