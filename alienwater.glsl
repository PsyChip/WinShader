...
vec3 skyColor(vec3 ro, vec3 rd) {
  ...
  vec3 planetColLocal = mix(1.3*planetCol, 0.3*planetCol, pow(psin(planetLat+1.0)*psin(sqrt(2.0)*planetLat+2.0)*psin(sqrt(3.5)*planetLat+3.0), 0.5));
  ...
  final += step(0.0, si.x)*pow(planetDiff, 0.75)*mix(planetColLocal, ringsCol, 0.0)*dustTransparency*borderTransparency + ringsCol*(1.0 - borderTransparency);
...
vec3 getColor(vec3 ro, vec3 rd) {
  ...
  if (id > 0.0) {
    ...
    float spe = pow(max(dot(sunDir, reflect(rd, nrm)), 0.0), 3.0);
    float fre = pow(1.0-dot(nrm, -rd), 2.0);
